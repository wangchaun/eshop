...
import com.sinokj.app.front.browse.service.BrowseBuyService;
...
public class FrontGoodAction extends BaseAction {
	...
	private SaleOrder saleOrder2;
	private List<Good>saleRanking;
	...
	public double getshoppingMoney() {
		double sumMoney = 0;
		ShoppingCar shoppingCar = ((ShoppingCar) getRequest().getSession().getAttribute("shoppingCar"));
		for (GoodsInCar car : shoppingCar.getGoodsInCarList()) {
			sumMoney = sumMoney + car.getTotalMoney();
		}
		return sumMoney;
	}

	public boolean getConfime(String keys) {
		String[] key = keys.split(",");
		return cardService.confimeatin(key[0], key[1]);
	}
	/* 使用一卡通 */
	public void useMaterialGiftCard() {
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		String keys = request.getParameter("giftCardKey");
		boolean flag = getConfime(keys);
		double sumMoney = getshoppingMoney();
		double money = getsessionMoney("cardmoney");
		double tempjd = getsessionMoney("jd");
		double total = tempjd + money;
		/* 判断是否验证输入正确 */
		if (tempjd != sumMoney) {
			if (flag) {
				ArrayList<String> cards = (ArrayList<String>) session.getAttribute("cards");
				ArrayList<String> cardmoneys = (ArrayList<String>) session.getAttribute("cardmoneys");
				if (cards == null) {
					cards = new ArrayList<String>();
					cardmoneys = new ArrayList<String>();
					money = cardService.getMoney();
					cards.add(keys);
					cardmoneys.add(String.valueOf(money));
					double pay = sumMoney - (money + tempjd);
					if (pay <= 0) {
						responseFlag(returnState(cards.size(), money, sumMoney- tempjd, 0));// 成功提示
					} else {
						responseFlag(returnState(cards.size(), money, money,pay));// 成功提示
					}
					session.setAttribute("cards", cards);
					session.setAttribute("cardmoneys", cardmoneys);
					session.setAttribute("cardmoney", String.valueOf(money));
				} else {
					double temp = tempjd;
					for (String card : cards) {
						getConfime(card);
						temp = temp + cardService.getMoney();
					}
					if (temp < sumMoney) {
						getConfime(keys);
						double cardmoney = cardService.getMoney();
						String tt = String.valueOf(cardmoney);
						cards.add(keys);
						cardmoneys.add(String.valueOf(tt));
						String tp = String.valueOf(money + cardmoney);
						double total2 = total + cardmoney;
						double pay = sumMoney - total2;
						if (total2 >= sumMoney) {
							responseFlag(returnState(cards.size(), cardmoney,
									sumMoney - tempjd, 0));// 成功提示
						} else {
							responseFlag(returnState(cards.size(), cardmoney,
									money + cardmoney, pay));// 成功提示
						}
						session.setAttribute("cards", cards);
						session.setAttribute("cardmoneys", cardmoneys);
						session.setAttribute("cardmoney", tp);
					} else {
						responseFlag("ok");// 积分+易啊开通满于支付商品,不需要使用消费卡。
					}
				}
			} else {
				responseFlag(false);// 用户名和密码错误。
			}
		} else {
			responseFlag("ok");// 京东满于支付商品,不需要使用消费卡。
		}

	}
	public void useMaterialGiftCard2() {
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		String keys = request.getParameter("giftCardKey");
		boolean flag = getConfime(keys);
		double sumMoney = getGoodMoney();
		double money = getsessionMoney("cardmoney2");
		double tempjd = getsessionMoney("jd2");
		double total = tempjd + money;
		/* 判断是否验证输入正确 */
		if (tempjd != sumMoney) {
			if (flag) {
				ArrayList<String> cards = (ArrayList<String>) session.getAttribute("cards2");
				ArrayList<String> cardmoneys = (ArrayList<String>) session.getAttribute("cardmoneys2");
				if (cards == null) {
					cards = new ArrayList<String>();
					cardmoneys = new ArrayList<String>();
					money = cardService.getMoney();
					...
					session.setAttribute("cards2", cards);
					session.setAttribute("cardmoneys2", cardmoneys);
					session.setAttribute("cardmoney2", String.valueOf(money));
				} else {
					double temp = tempjd;
					for (String card : cards) {
						getConfime(card);
					...
					if (temp < sumMoney) {
						...
						session.setAttribute("cards2", cards);
						session.setAttribute("cardmoneys2", cardmoneys);
						session.setAttribute("cardmoney2", tp);
					} else {
						responseFlag("ok");// 积分+易啊开通满于支付商品,不需要使用消费卡。
					}
				}
	...
	/* 根据状态返回 */
	public String returnState(int i, double cardMoney, double cardTotal,double pay) {
		String cardNum = String.valueOf(i);
		String cardtotals = String.valueOf(String.valueOf(cardTotal));
		String cardmm = String.valueOf(cardMoney);
		String pays = String.valueOf(pay);
		return cardNum + "," + cardmm + "," + cardtotals + "," + pays;
	}

	/* 取消一卡通 */
	public void cancelMaterialGiftCard() {
		/*
		 * 1获取session集合当中的卡 2遍历集合中卡
		 * 3根据输入框的帐号和密码查找卡,如果卡号符合，则遍历出session集合当中的卡，与之相对应的卡删除。
		 */
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		double sumMoney = getshoppingMoney();
		...
		if (flag) {
			...
			if (cards.size() == 0) {
				...
				session.removeAttribute("cardmoney");
				session.removeAttribute("cardmoneys");
				responseFlag(returnState(0, 0, 0, sumMoney - tempjd));// 成功提示
				...
				double pay = sumMoney - (money + tempjd);
				if (pay >= 0) {
					responseFlag(returnState(cards.size(), 0, money, pay));// 成功提示
				} else {
					responseFlag(returnState(cards.size(), 0,
							sumMoney - tempjd, 0));// 成功提示
				}
			}
		} else {
	...
	public void cancelMaterialGiftCard2() {
		/*
		 * 1获取session集合当中的卡 2遍历集合中卡
		 * 3根据输入框的帐号和密码查找卡,如果卡号符合，则遍历出session集合当中的卡，与之相对应的卡删除。
		 */
		...
		if (flag) {
			...
			if (cards.size() == 0) {
				session.removeAttribute("cards2");
				session.removeAttribute("cardmoney2");
				session.removeAttribute("cardmoneys2");
				responseFlag(returnState(0, 0, 0, sumMoney - tempjd));// 成功提示
	...
	/** 从session取到相应的值 */
	public double getsessionMoney(String sessionName) {
		String temp = (String) (getRequest().getSession().getAttribute(sessionName));
		if (temp == null) {
			temp = "0";
		}
	...
	/* 查找积分数量 */
	public void searchCreadit() {
		Customer loginMan = getSessionCustomerInfo();
		HttpServletRequest request = getRequest();
		int credit = Integer.valueOf(request.getParameter("credit"));
		int credit2 = loginMan.getCredit().intValue();
		if (credit > credit2) {
			responseFlag(String.valueOf(credit2));
		} else {
			responseFlag(true);
		}
	}
	...
	public void userBean() {
		...
		double money = getsessionMoney("cardmoney");
		int credit = Integer.valueOf(request.getParameter("credit"));
		double cc =Double.parseDouble(df.format( credit*CreditUtil.getUnit()));
		
		double jd = (money + cc) - sumMoney;
		if (customer.getCredit() < credit) {
			...
			if (jd > 0) {
				...
				session.setAttribute("jd", String.valueOf(cc));
				responseFlag(String.valueOf(cc) + "," + String.valueOf(-jd));
			}
		}
	}
	...
	public void CancelBean() {
		...
		double sumMoney = getshoppingMoney();
		double money = getsessionMoney("cardmoney");
		double result = sumMoney - money;
		session.removeAttribute("jd");
		if (result > 0) {
			responseFlag(String.valueOf(result));
		} else {
			responseFlag("0");
		}
	}
	...
	public double  getGoodMoney(){
		Good good=(Good)getRequest().getSession().getAttribute("telphone");
		return good.getPrice();
	}
	public void userBeanTwo() {
		this.customer = getSessionCustomerInfo();
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		DecimalFormat df = new DecimalFormat("#.##");
		...
		if (customer.getCredit() < credit) {
			...
			if (jd > 0) {
				...
				session.setAttribute("jd2", String.valueOf(cc));
				responseFlag(String.valueOf(cc) + "," + String.valueOf(-jd));
			}
		}
	}
	/* 取消积分 */
	public void CancelBean2() {
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		double sumMoney = getGoodMoney();
		double money = getsessionMoney("cardmoney2");
		double result = sumMoney - money;
		session.removeAttribute("jd2");
		if (result > 0) {
			responseFlag(String.valueOf(result));
		} else {
			responseFlag("0");
	...
	public String brand_json() {
		...
		List gbList = this.goodBrandService.select(this.goodBrand);
	...
	public String miaosha() {
		...
		this.information = ((Information) this.informationService
				.selectOne(this.information));
		if (this.good == null) {
			this.good = new Good();
		}
	...
	public String getLeve_3Page() throws Exception {
		this.goodType = ((GoodType) this.goodTypeService.getModel(this.goodType
				.getId()));
		...
		GoodType goodType2 = this.goodTypeService.getLevelTypeNo(this.goodType
				.getId());
		...
		this.goodArrList = this.goodService
				.select("Good.Good_query", this.good);
		if (this.goodArrList != null) {
			((Good) this.goodArrList.get(0)).setIndexs(String
					.valueOf(this.goodArrList.size()));
		}
		...
		if (this.brandlist != null) {
			this.goodBrand = new GoodBrand();
			this.goodBrand.setParentId(((GoodBrand) this.brandlist.get(0))
					.getId());
			this.brandlist = this.goodBrandService.select(this.goodBrand);
		}
	...
	public String productJson() {
		...
		if (this.good.getGoodTypeId() != null) {
			this.goodType = ((GoodType) this.goodTypeService.getModel(this.good
					.getGoodTypeId()));
		}
	...
	public String warmlist() {
		...
		try {
			...
			this.promoteGoodList = this.promoteGoodService
					.getPromoteGoodByPromoteId(promoteId);
			if (this.promoteGoodList.size() > 0) {
				Good good = new Good();
				for (int i = 0; i < this.promoteGoodList.size(); ++i) {
					good = (Good) this.goodService
							.getModel(((PromoteGood) this.promoteGoodList
									.get(i)).getGoodId());
					((PromoteGood) this.promoteGoodList.get(i)).setPic(good
							.getPic());
					((PromoteGood) this.promoteGoodList.get(i)).setPicId(good
							.getPicId());
				}
			}
			this.promotePicList = this.fileUploadService
					.getFileUploadById(promoteId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "warm";
	...
	public String getGoodsById() throws Exception {
		...
		if (this.good.getId() != null) {
			...
			if ((type != null) && (type.getLevel().intValue() == 1)) {
				...
				request.setAttribute("goodTypeName",this.good.getGoodTypeName());
			}
			...
			/*根据商品goodtype查询所有*/
			this.saleRanking=this.goodService.select("Good.Good_rankingList",this.good);
		...
		if (StringUtils.isNotBlank(this.good.getId())) {
			...
			if ((this.goodType3 != null)
					&& (this.goodType3.getParentId() != null)
					&& (!"".equals(this.goodType3.getParentId()))) {
				this.goodType2 = ((GoodType) this.goodTypeService
						.getModel(this.goodType3.getParentId()));
				arr.append(" " + this.goodType2.getRemark());

				if ((this.goodType2.getParentId() != null)
						&& (!"".equals(this.goodType2.getParentId()))) {
					this.goodType1 = ((GoodType) this.goodTypeService
							.getModel(this.goodType2.getParentId()));
		...
		this.list = this.goodSpecificationValService.select(
				"GoodSpecificationVal.GoodSpecificationVal_good_query",
		...
		for (int i = 0; i < goodSpecificationValList.size(); ++i) {
			...
			GoodSpecification specif = (GoodSpecification) this.goodSpecificationService
					.getModel(((GoodSpecificationVal) goodSpecificationValList
							.get(i)).getGoodSpecificationId());
		...
		if (str.length > 0) {
			...
			this.list1 = this.goodSpecificationService.select("GoodSpecification.GoodSpecification_specificationVal_query",
							goodSpecification);
		}
		try {
		...
		try {
			...
			this.goodComposeList = this.goodComposeService
					.getGoodComposeByGoodId(this.good.getId());
			if (this.goodComposeList.size() > 0) {
				for (int i = 0; i < this.goodComposeList.size(); ++i) {
					Good goods = new Good();
					goods.setId(((GoodCompose) this.goodComposeList.get(i))
							.getComposeGoodId());
					List lister = this.goodService
							.select("Good.Good_SY", goods);
					if (lister.size() > 0) {
						((GoodCompose) this.goodComposeList.get(i))
								.setGoodName(((Good) lister.get(0)).getName());
						((GoodCompose) this.goodComposeList.get(i))
								.setPic(((Good) lister.get(0)).getPic());
						((GoodCompose) this.goodComposeList.get(i))
								.setIntroBrief(((Good) lister.get(0))
										.getIntroBrief());
					}
				}
			}
			getRequest().setAttribute("goodComposeSize",
					Integer.valueOf(this.goodComposeList.size()));
			getRequest().setAttribute("goodSpecificationnumber",
					Integer.valueOf(this.list1.size()));
		} catch (Exception e) {
	...
	public void selectGoodStock() {
		...
		try {
			...
			String temp=this.ware.getWareSpecificationVal();
			if (temp!= null) {
				 String [] con = this .ware . getWareSpecificationVal() . split( ",") ;
                 this.ware. setWareSpecificationValId (con );
                 this.ware= this .wareService . selectWareStock( this. ware) ;
                 int stock = this. ware. getStock() ;
                 if ( stock > 0) {
                      responseString = stock + ","+this.ware.getPriceMarket()+","+this.ware.getPriceShop()+"," +temp ;
                 } else {
                      responseFlag ("error" );
                 }
	...
	public String getGoodMessageJson() {
		...
		resultList = this.messageService.pageList(this.pageInfo, this.message,
				true);
	...
	public String getWareOfSpecificationJson() {
		...
		if (this.goodSpecification.getGoodId() != null) {
			List wareArrList = new ArrayList();
			this.ware = new Ware();
			...
			for (int i = 0; i < wareArrList.size(); ++i) {
				...
				List goodSpecificationValList = this.goodSpecificationValService.select("GoodSpecificationVal.GoodSpecificationVal_ware_query",
								this.goodSpecificationVal);
				String str = "";
				boolean to = true;
				for (int j = 0; j < goodSpecificationValList.size(); ++j) {
					str = ((GoodSpecificationVal) goodSpecificationValList
							.get(j)).getValue();
					boolean too = false;
	...
	public void addGoodToCart() {
		...
		try {
			...
			List listgood = this.goodService.select("Good.Good_SY", this.good);

			if ((listgood != null) && (listgood.size() > 0))
				this.good = ((Good) listgood.get(0));
			this.ware = ((Ware) this.wareService.getModel(wareId));
			this.warehousecount = ((Integer) this.goodService.selectToT(
					"Ware.Ware_warehousecount_query", this.ware));
			if ((this.warehousecount != null)
					&& (this.warehousecount.intValue() < 1)) {
				responseFlag("NoWare");
				return;
			}
			if ((this.warehousecount != null)
					&& (this.warehousecount.intValue() > 0)
			...
			System.out.println(this.ware.getPriceShop()==null);
			...
			if (this.ware != null) {
				...
				if(this.ware.getPriceShop()==null){
				    ...
				    this.goodInCar.setPrice(this.ware.getPriceShop());
				    this.goodInCar.setTotalMoney(Double.valueOf(this.ware.getPriceShop().doubleValue()* count.intValue()));
				}
	...
	public void addGoodseewareToCart() {
		...
		try {
			...
			if ((this.ware != null) && (listgood.size() > 0)) {
				...
				List listsv = this.goodSpecificationValService.select(
						"GoodSpecificationVal.GoodSpecificationVal_ware_query",
						goodSpecificationVals);

				for (int j = 0; j < listsv.size(); ++j) {
					wareSpecificationVal = wareSpecificationVal
							+ ((GoodSpecificationVal) listsv.get(j)).getValue()
							+ ",";
					if (((GoodSpecificationVal) listsv.get(j)).getValue()
							.equals("上门安装")) {
				...
				this.goodInCar.setPicId(this.good.getPicId());
				this.goodInCar.setPic(this.good.getPic());
				this.goodInCar.setTotalMoney(Double.valueOf(this.good
				...
				if (this.goodsInCarList.size() > 0) {
					for (int j = 0; j < this.goodsInCarList.size(); ++j) {
						GoodsInCar good = (GoodsInCar) this.goodsInCarList
								.get(j);
						if (good.getId().equals(this.goodInCar.getId())) {
							good.setCount(Integer.valueOf(good.getCount()
									.intValue()
									+ count.intValue()));

							Double itemMoney = Double.valueOf(good.getCount()
									.intValue()
	...
	public String getshoppingCar() {
		...
		HttpSession session = request.getSession();
		Object obj = session.getAttribute("shoppingCar");
		int number = ((Integer) session.getAttribute("shoppingCarSize"))
				.intValue();
		if (obj == null)
	...
	public void addGoodComposeToCart() {
		...
		try {
			...
			if (this.good != null) {
				this.goodInCar = new GoodsInCar();
				List wareList = this.wareService.getWareByGoodId(this.good
						.getId());
				this.goodInCar.setAreaName(area.getName());
				this.goodInCar = new GoodsInCar();
				...
				if (this.goodCompose.getGoodId() != null) {
					String[] goodIdArr = this.goodCompose.getGoodId()
							.split(",");
					for (int j = 0; j < goodIdArr.length; ++j) {
						...
						if ((goodId != null) && (!goodId.equals(""))) {
							Double groupPrice = Double.valueOf(0.0D);
							this.goodComposeList = this.goodComposeService
									.getGoodComposeOfGoodId(goodId);
							if (this.goodComposeList.size() > 0) {
								String composeGoodId = ((GoodCompose) this.goodComposeList
										.get(0)).getComposeGoodId();
								Good goods = (Good) this.goodService
										.getModel(goodId);
								List wareLists = this.wareService
										.getWareByGoodId(goodId);
								groupPrice = ((GoodCompose) this.goodComposeList
										.get(0)).getPriceDiscount();

								this.goodInCar = new GoodsInCar();
								this.goodInCar.setAreaName(area.getName());
								this.goodInCar.setId(((Ware) wareLists.get(0))
										.getId());
								this.goodInCar.setCode(goods.getCode());
								this.goodInCar.setName(goods.getName());

								this.goodInCar.setPrice(Double.valueOf(goods
										.getPrice().doubleValue()
										- groupPrice.doubleValue()));
								...
								this.goodInCar.setTotalMoney(Double
										.valueOf(goods.getPrice().doubleValue()
												- groupPrice.doubleValue()));
								boolean sameFlags = false;

								if (this.goodsInCarList.size() > 0) {
									for (int m = 0; m < this.goodsInCarList
											.size(); ++m) {
										GoodsInCar good = (GoodsInCar) this.goodsInCarList
												.get(m);
										if (good.getId().equals(
												this.goodInCar.getId())) {
											good
													.setCount(Integer
															.valueOf(good
																	.getCount()
											...
											Double itemMoney = Double
													.valueOf(good.getCount()
															.intValue()
															* (goods
																	.getPrice()
																	.doubleValue() - groupPrice
																	.doubleValue()));
											good.setTotalMoney(itemMoney);
											sameFlag = true;
										}
								...
								if (!sameFlags) {
									number += this.goodInCar.getCount()
											.intValue();
									this.goodsInCarList.add(this.goodInCar);
								}
							}
	...
	public void addGoodToFavorite() {
		...
		if (this.customer == null)
			...
			try {
				...
				Favorite f = (Favorite) this.favoriteService
						.selectOne(this.favorite);
				if (f == null) {
					String goodId = this.favorite.getGoodId();
					if ((goodId == null) && (this.favorite.getWareId() != null)) {
						Ware ware = (Ware) this.wareService
								.getModel(this.favorite.getWareId());
	...
	public String myShoppingCar() {
		...
		if (this.shoppingCar.getGoodsInCarList() == null)
			...
			session.setAttribute("shoppingCarSize", Integer
					.valueOf(this.shoppingCar.getGoodsInCarList().size()));
		}
		...
		if (this.good != null) {
			List goodArray = new ArrayList();
			this.good.setLevel(Integer.valueOf(3));
			this.good.setIsInventory("0");
			this.good.setAreasId(areaId);
			this.good.setSalesVolume("sales");
			goodArray = this.goodService.select("Good.Good_SYgoodType",
					this.good);
			this.goodList = new ArrayList();

			int n = 0;
			while ((this.goodList.size() < 10)
					&& (this.goodList.size() < goodArray.size()) && (n < 500)) {
				Random random = new Random();
				int num = random.nextInt(51);
				if ((num < 50) && (num < goodArray.size())) {
					boolean ser = true;
					for (int i = 0; i < this.goodList.size(); ++i) {
						if (!((Good) this.goodList.get(i)).getId().equals(
								((Good) goodArray.get(num)).getId()))
							continue;
						ser = false;
					}
					if (ser)
	...
	public void XGSpecificationVal() {
		...
		for (int i = 0; i < this.goodsInCarList.size(); ++i)
			if (((GoodsInCar) this.goodsInCarList.get(i)).getId().equals(
					wareIds)) {
				String str = ((GoodsInCar) this.goodsInCarList.get(i))
						.getSpecificationVal();
				...
				((GoodsInCar) this.goodsInCarList.get(i))
						.setIsServices(goodsIncarselect);
				((GoodsInCar) this.goodsInCarList.get(i))
						.setSpecificationVal(newstr);
				shoppingCar.setGoodsInCarList(this.goodsInCarList);
				session.setAttribute("shoppingCar", shoppingCar);
				responseFlag(true);
			} else {
	...
	public void deleteGoodFromCar() {
		...
		for (int i = 0; i < this.goodsInCarList.size(); ++i) {
			...
			if (goodId
					.equals(((GoodsInCar) this.goodsInCarList.get(i)).getId())) {
	...
	public void changeNum() {
		...
		if (countStr != null) {
			...
			if (count.intValue() <= this.warehousecount.intValue()) {
				if (this.goodsInCarList.size() > 0) {
					for (int j = 0; j < this.goodsInCarList.size(); ++j) {
						if (((GoodsInCar) this.goodsInCarList.get(j)).getId()
								.equals(goodId)) {
							((GoodsInCar) this.goodsInCarList.get(j))
									.setCount(count);
							((GoodsInCar) this.goodsInCarList.get(j))
									.setTotalMoney(Double.valueOf(count
											.intValue()
											* ((GoodsInCar) this.goodsInCarList
													.get(j)).getPrice()
													.doubleValue()));
						}
					}
	...
	public void isNotEmptyCart() {
		...
		if (obj == null) {
			...
			this.shoppingCar = ((ShoppingCar) obj);
			if ((this.shoppingCar.getGoodsInCarList() != null)
					&& (!this.shoppingCar.getGoodsInCarList().isEmpty()))
				responseFlag(true);
			else
				responseFlag(false);
		}
	...
	private void cleanShoppingCar() {
		if (this.checked != null) {
			...
			getRequest().getSession().setAttribute("shoppingCarSize",
					Integer.valueOf(checked1.length));
		}
	}
	...
	public String jiesuan2() throws Exception {
		HttpServletRequest request = getRequest();
		HttpSession session = getRequest().getSession();
		addressList();
		this.informationList = this.informationService.select(new Information());
		...
		if (this.customerAddress != null) {
			...
			str = this.customerAddress.getName() + " "
					+ this.customerAddress.getAddress() + " "
					+ this.customerAddress.getStreet() + ",邮编"
        ...
        String[] wareIdArr1 = new String[1];
		wareIdArr1[0] = this.ware.getId();
		ware.setId(wareIdArr1[0]);
		/**
		 * 1根据wareid找到goodid 2根据goodid找到good
		 */
		List<Ware> wares = this.wareService.select(ware);
		String goodId = wares.get(0).getGoodId();
		String username = request.getParameter("username");
		String cardNo = request.getParameter("cardNo");
		String month=request.getParameter("month");
		String packageType=request.getParameter("packageType");
		String cardkey = this.phone.getTelphone();
		String compond=username + "," + cardNo + ","+ cardkey+","+packageType+","+month;
		session.setAttribute("username", compond);
		session.setAttribute("cardkey",cardkey);
		if (this.good == null) {
			good = new Good();
		}
		good.setId(goodId);
		List<Good> goods = this.goodService.select(good);
		good = goods.get(0);
		session.setAttribute("telphone", good);
		session.setAttribute("wareId", wareIdArr1[0]);
		/** ************************************************ */
		this.goodType = new GoodType();
		this.goodType.setWareIdArr(wareIdArr1);
		...
		for (int j = 0; j < goodTypeList.size(); ++j) {
			...
			if (paymentId.equals(""))
				...
				paymentId = getintersection(paymentId, this.goodType
						.getPaymentId());
			}
			if (deliveryId.equals(""))
				deliveryId = this.goodType.getDeliveryId();
			else {
				deliveryId = getintersection(deliveryId, this.goodType
						.getDeliveryId());
			}
		}
	...
	public String jiesuan() throws Exception {
		...
		if ((this.shoppingCar != null)
				&& (this.shoppingCar.getGoodsInCarList() != null)
			...
			for (int i = 0; i < this.goodsInCarList.size(); ++i) {
				wareIdArr[i] = ((GoodsInCar) this.goodsInCarList.get(i))
						.getId();
			}

			this.goodType = new GoodType();
			this.goodType.setWareIdArr(wareIdArr);
			List<GoodType> goodTypeList = this.goodTypeService
					.selectDelAndPays(this.goodType);
			String paymentId = "";
			String deliveryId = "";
			for (int j = 0; j < goodTypeList.size(); ++j) {
				this.goodType = ((GoodType) goodTypeList.get(j));
			...
			List<SaleOrder> listorder = this.saleOrderService.select(
					"SaleOrder.SaleOrder_customer", this.order);
			if (listorder.size() > 0) {
	...
	public void saveOrderAddress() {
		...
		try {
			/*******************************************************************
			 * if (this.customerAddress == null) { this.customerAddress = new
			 * CustomerAddress(); }
			 ******************************************************************/
			this.addressList = this.customerAddressService.select(address);
			if (this.customerAddress.getId() == null) {
				List<CustomerAddress> list = this.customerAddressService
						.select(address);
				if (this.addressList.size() < 10) {
					if ((list != null) && (list.size() > 0)) {
						for (int i = 0; i < list.size(); ++i) {
							((CustomerAddress) list.get(i))
									.setWhetherordrer("1");
							this.customerAddressService.update((Base) list
									.get(i));
						}
					}
	...
	public void delectcustomerAddress() {
		if (this.customerAddress != null)
			if (this.customerAddress.getId() != null) {
				try {
					this.customerAddressService.delete(this.customerAddress
							.getId());
				} catch (Exception e) {
					e.printStackTrace();
					responseFlag(false);
	...
	public void srachCoupon() {
		...
		try {
			...
			this.vipCoupon.setState("s");
			this.vipCoupon.setCustomerId(this.customer.getId());
			this.vipCoupon = ((VipCoupon) this.vipCouponService
					.selectOne(this.vipCoupon));
			if (this.vipCoupon == null) {
				responseFlag(false);
			} else {
				String infor = this.vipCoupon.getId() + ","
						+ this.vipCoupon.getName() + ","
						+ this.vipCoupon.getMinBuy() + ","
						+ this.vipCoupon.getMoney();
				responseFlag(infor);
			}
		} catch (Exception e) {
			responseFlag(false);
	...
	/* 积分，卡号添加到指定表中 */
	private void create(double jd, double cardMoney, double summoney,ArrayList<String> cards,String saleOrder) {
		SaleOrderPayService service=new SaleOrderPayService();
		service.create(jd, cardMoney, summoney, cards, saleOrder);
	}
	/*创建订单*/
	public void createOrder(){
		...
		if ((this.shoppingCar != null)&& (this.shoppingCar.getGoodsInCarList() != null)
			...
			boolean flag=false;
			try {
				flag = this.frontGoodService.createSaleOrder(this.saleOrder, this.shoppingCar.getGoodsInCarList(), this.customer, this.vipCoupon);
			}catch (Exception e){
				logger.error("createOrder==={}",e);
			}
			if (flag) {
				/* 消费积分,卡号金额记录到表中 */
				//if (jd != 0.0 || cardMoney != 0.0) {
					//ArrayList<String> cards = (ArrayList<String>) session.getAttribute("cards");
					//create(jd, cardMoney, summoney, cards, saleOrder.getCode());
				//}
				//this.customer.setCredit(this.customer.getCredit()-(int)(jd*CreditUtil.getMinMoney()));
				//this.customerService.update(this.customer);
				
				//double temp2=summoney-(cardMoney+jd);
				//if(temp2<=0.0){/*积分和一卡通金额总和总于支付商品,就更新库存,修改用户的积分数量*/
					/*修改仓库数量和订单状态*/
					//cardService.updategoodNumber(saleOrder.getId());
					//saleOrder.setPaymentState("1");
					//this.saleOrderService.update(saleOrder);
					/*修改积分数量*/
					//this.customer.setCredit(this.customer.getCredit()+CreditUtil.getCredit1(summoney));
					//this.customerService.update(this.customer);
					//responseFlag("ok");//直接跳转成功页面。
				//}else{
					//responseFlag(flag);//需要用到付宝来支付。
				//}
				/* 生成订单后，清除购物车,清除浏览器绑定的session值*/
				ShoppingCar shoppingCar = new ShoppingCar();
				shoppingCar.setGoodsInCarList(new ArrayList<GoodsInCar>());
				session.setAttribute("saleOrder", saleOrder);
				session.setAttribute("shoppingCar", shoppingCar);
				session.setAttribute("shoppingCarSize", 0);
				session.removeAttribute("cardmoney");
				session.removeAttribute("jd");
				session.removeAttribute("cards");
				responseFlag(flag);
	...
	public void createOrder2() throws Exception {
		...
		HttpServletRequest request = getRequest();
		HttpSession session = request.getSession();
		double jd = getsessionMoney("jd2");
		double cardMoney = getsessionMoney("cardmoney2");
		double summoney = getGoodMoney();
		String username = (String) session.getAttribute("username");
		String wareId = (String) session.getAttribute("wareId");
		double temp=summoney-(cardMoney+jd);
		if(temp<0){
			temp=summoney;
		}else{
			temp=cardMoney+jd;				
		} 	
		Good good = (Good) session.getAttribute("telphone");
		if (good != null) {
			boolean flag = this.frontGoodService.createSaleOrder(this.saleOrder, temp, this.customer, this.vipCoupon, good,wareId, username);
			if (flag) {
				/* 消费积分,卡号金额记录到表中 */
				if (jd != 0.0 || cardMoney != 0.0) {
					ArrayList<String> cards = (ArrayList<String>) session.getAttribute("cards2");
				...
				double temp2=summoney-(cardMoney+jd); 
				if(temp2<=0.0){/*积分和一卡通金额总和总于支付商品,就更新库存,修改用户的积分数量*/
					String telphone=(String) session.getAttribute("cardkey");
					/*修改仓库数量和订单状态*/
					cardService.updategoodNumber(saleOrder.getId());
					saleOrder.setPaymentState("1");
					this.saleOrderService.update(saleOrder);
					...
					this.phoneService.select(this.phone);
				...
				/* 生成订单后，清除购物车,清除浏览器绑定的session值*/
				session.setAttribute("saleOrder2", this.saleOrder);
				session.removeAttribute("username");
				session.removeAttribute("telphone");
				session.removeAttribute("wareId");
				session.removeAttribute("moneyTwo");
				session.removeAttribute("cardmoney2");
				session.removeAttribute("jd2");
				session.removeAttribute("cards2");
	...
	public String successOrder2() {
		...
		SaleOrder saleOrder2 = (SaleOrder) session.getAttribute("saleOrder2");
		try {
			List<Good> goodSortList = this.goodService.getGoodsByTotalsales();
			request.setAttribute("goodSortList", goodSortList);
		} catch (Exception e) {
		...
		request.setAttribute("saleOrder2", saleOrder2);
	...
	public void reportComment() {
		...
		if (this.customer == null) {
			...
			try {
				...
				List saleWareIdList = this.saleWareService
						.getSaleWareByCustomerId(this.customer.getId());

				List wareList = this.wareService.getWareByGoodId(goodId);
				if (wareList.size() > 0) {
					for (int i = 0; i < wareList.size(); ++i) {
						if (saleWareIdList.contains(((Ware) wareList.get(i))
								.getId())) {
							isFlag = true;
							break;
						}
	...
	public void wareCommentSave() {
		...
		if (this.customer == null) {
			...
			List list = this.goodService.select("Good.Good_select_buy", goods);
			if (list.size() == 0) {
	...
	public void replyCommentSave() {
		...
		if (this.customer == null) {
			...
			this.wareComment.setCreateTime(new Date());
			try {
				int commentCount = this.wareCommentService.count(
						this.wareComment.getCommentId()).intValue();
	...
	public String moreComment() {
		...
		try {
			...
			this.goodSalesList = this.goodService.getGoodsByType(this.good
					.getGoodTypeId());
			getRequest().setAttribute("goodSalesSize",
					Integer.valueOf(this.goodSalesList.size()));
			this.wareComment.setWareId(goodId);

			this.wareCommentList = this.wareCommentService.pageList(pageInfo,
					this.wareComment, true);
			this.repalyCommentList = this.wareCommentService.getCountComment(
					goodId, "3");
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			this.goodRelateList = this.goodRelateService
					.getGoodRelateByGoodId(this.good.getId());
			if (this.goodRelateList.size() > 0) {
				for (int i = 0; i < this.goodRelateList.size(); ++i) {
					String relateGoodId = ((GoodRelate) this.goodRelateList
							.get(i)).getRelateGoodId();
					Good good = (Good) this.goodService.getModel(relateGoodId);
					((GoodRelate) this.goodRelateList.get(i)).setGoodName(good
							.getName());
					((GoodRelate) this.goodRelateList.get(i)).setPic(good
							.getPic());
					((GoodRelate) this.goodRelateList.get(i)).setPrice(good
							.getPrice());
				}
			}
			getRequest().setAttribute("goodRelateSize",
					Integer.valueOf(this.goodRelateList.size()));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String showRepalyMessage() {
		try {
			...
			this.message = this.messageService.getRepalyById(this.message
					.getId());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String moreMessage() {
		...
		try {
			...
			this.messageList = this.messageService.pageList(pageInfo,
					this.message, true);

			this.repalyMessageList = this.messageService
					.getReplyMessageByGoodId(goodId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
	...
	public String serchGoods() {
		...
		this.informationList = this.informationService
				.select(new Information());
		this.goodList = this.frontGoodService.pageList(pageInfo, this.good,
				true);
	...
	public String listPromotegood() {
		...
		try {
			this.promoteGoodlist = this.promoteGoodService.pageList(pageInfo,
					this.promoteGood, true);
			if (this.promoteGoodlist.size() > 0) {
				Good good = new Good();
				for (int i = 0; i < this.promoteGoodlist.size(); ++i) {
					good = (Good) this.goodService
		...
		if (this.good != null) {
			if (this.good.getGoodTypeId() != null) {
				try {
					...
					GoodType goodt = (GoodType) this.goodTypeService
							.getModel(this.good.getGoodTypeId());
					if (goodt != null) {
						if (goodt.getLevel().intValue() != 1) {
							this.goodType.setParentId(goodt.getParentId());
							this.goodTypeList = this.goodTypeService
									.getparentIdGoodType(this.goodType
											.getParentId());
						}
						if (goodt.getLevel().intValue() == 1) {
			...
			if (this.good.getBrandId() != null) {
				try {
					...
					GoodBrand goodb = (GoodBrand) this.goodBrandService
							.getModel(this.good.getBrandId());
					if (goodb != null) {
						if (goodb.getLevel().intValue() != 1) {
							this.goodBrand.setParentId(goodb.getParentId());
							list = this.goodBrandService
									.getparentIdGoodBrand(this.goodBrand
											.getParentId());
						}
						if (goodb.getLevel().intValue() == 1) {
							if (list == null) {
		...
		if ((this.good != null) && (this.good.getId() != null)) {
			...
			try {
				this.goodComposeList = this.goodComposeService
						.getGoodComposeByGoodId(this.good.getId());
				if (this.goodComposeList.size() > 0) {
					((GoodCompose) this.goodComposeList
							.get(this.goodComposeList.size() - 1))
							.setRangeStr("dasdascc");
	...
	public String getWarehouseList() {
		try {
			List list = this.warehouseService.getWarehouseList();
			if (list.size() > 0) {
				JSONArray jsarry = new JSONArray();
				jsarry = JSONArray.fromObject(list);
				this.resultTree = jsarry;
	...
	public String getwareCommentJson() {
		try {
			...
			if (this.good.getId() != "") {
				...
				if (ware != null) {
					this.wareCommentList = this.wareCommentService
							.getCountComment(ware.getId(), this.good
									.getAppType());
				}
			}
			...
			this.jsonMap = new HashMap();
			this.jsonMap.put("total", Integer.valueOf(this.wareCommentList
					.size()));
	...
	public String pingjia2() {
		HttpServletRequest request = getRequest();
		if (this.wareComment == null) {
			this.wareComment= new WareComment();
		}
		request.setAttribute("goodId", this.wareComment.getWareId());
		request.setAttribute("code", this.wareComment.getCode());
		this.advertiseList = this.advertiseService.getAdvertiseList();
		return "pingjia";
	}
		...
		if ((this.doWhat != null) && (StringUtils.isNotBlank(this.doWhat))) {
			try {
				if (this.doWhat.equals("over")) {
					...
					g.setIsInventory("2");
					g.setIsNext("0");
					List<Good> gList = this.goodService.select(g);
					for (Good go : gList) {
						go.setIsInventory("0");
	...
	public String orderpay() {
		if (getRequest().getParameter("orderId") != null) {
			this.saleOrder = ((SaleOrder) this.saleOrderService
					.getModel(getRequest().getParameter("orderId")));
		}
	...
	public void setGoodSpecificationService(
			GoodSpecificationService goodSpecificationService) {
		this.goodSpecificationService = goodSpecificationService;
	}

	public void setGoodSpecificationValService(
			GoodSpecificationValService goodSpecificationValService) {
	...
	public void setGoodSpecificationVal(
			GoodSpecificationVal goodSpecificationVal) {
	...
	public void setWareSpecificationVal(
			WareSpecificationVal wareSpecificationVal) {
	...
	public SaleOrder getOrder2() {
		return order2;
	}

	public void setOrder2(SaleOrder order2) {
		this.order2 = order2;
	}

	public SaleOrder getSaleOrder2() {
		return saleOrder2;
	}

	public void setSaleOrder2(SaleOrder saleOrder2) {
		this.saleOrder2 = saleOrder2;
	}

	public List<Good> getSaleRanking() {
		return saleRanking;
	}

	public void setSaleRanking(List<Good> saleRanking) {
