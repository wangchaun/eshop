...
function getColumnsOpt(){
	var opt = [
		...
		{field:'moneyAccount',title:'应收金额',width:25,align:'left',formatter:moneyAccountFormat},
		{field:'moneyCollect',title:'实收金额',width:25,align:'left',formatter:moneyCollectFormat},
		{field:'customerName',title:'付款单位',width:25,align:'left'},
		{field:'state',title:'收款单状态',width:25,align:'left',formatter:stateFormat},
...
//Json加载数据路径
function getUrlOpt(){
	var url = ctx+'/Collection_json!listJosn.do';
	return url;
}	

//编辑数据
function editData(id){
	var url = ctx+'/collection!edit.do?1=1';
	var title = '新增收款单';
	if(id != ''){
...
function realoadGrid(Code,customerName){
	...
	queryParams={"collection.code":Code,"collection.customerName":customerName};
...
//付款金额
function moneyCollectFormat(value,rowData,rowIndex){
	var moneyCollect = rowData.moneyCollect;
	if(moneyCollect){
		moneyCollect = moneyCollect.toFixed(2);
	}
	return moneyCollect;
}
