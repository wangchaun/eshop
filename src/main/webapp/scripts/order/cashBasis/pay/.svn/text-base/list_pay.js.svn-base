...
function getColumnsOpt(){
	var opt = [
		...
		{field:'moneyAccount',title:'应付金额',width:25,align:'left',formatter:moneyAccountFormat},
		{field:'moneyPayment',title:'实付金额',width:25,align:'left',formatter:moneyPaymentFormat},
		{field:'customerName',title:'收款单位',width:25,align:'left'},
		{field:'state',title:'付款单状态',width:25,align:'left',formatter:stateFormat},
...
//Json加载数据路径
function getUrlOpt(){
	var url = ctx+'/Pay_json!listJosn.do';
	return url;
}	

//编辑数据
function editData(id){
	var url = ctx+'/pay!edit.do?1=1';
	var title = '新增付款单';
	if(id != ''){
		url += '&pay.id='+id;
		title = '编辑付款单';
...
function realoadGrid(Code,customerName){
	...
	queryParams={"pay.code":Code,"pay.customerName":customerName};
...
//付款金额
function moneyPaymentFormat(value,rowData,rowIndex){
	var moneyPayment = rowData.moneyPayment;
	if(moneyPayment){
		moneyPayment = moneyPayment.toFixed(2);
	}
	return moneyPayment;
}
