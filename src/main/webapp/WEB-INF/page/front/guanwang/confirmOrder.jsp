...
$(document).ready(function(){
   ...
   <% ArrayList<String>cardss=(ArrayList<String>)session.getAttribute("cardmoneys");%>
...
function useOrCancelGiftCard(key,giftCardType,flag) {
    ...
    if(flag==0){
      url=ctx+"/cancelMaterialGiftCard.do";
      jQuery.ajax( {
		type : "POST",
		url : url,
		data : param,
        ...
        url=ctx+"/useMaterialGiftCard.do";
	    jQuery.ajax( {
		type : "POST",
		url : url,
		data : param,
...
<!--[if !IE]>和确认订单中的加盟店长是一样的<![endif]-->
  ...
  <div class="content">
      ...
      <table width="1199" border="0" cellspacing="0" cellpadding="0">
        ...
        <c:forEach items="${goodsInCarList}" var="good">
	        ...
	        <tr>
	          ...
	          <td width="139" align="center" valign="middle" class="itemListbg"><span class="font">¥<span>${good.totalMoney}</td>
	        </tr>
        </c:forEach>
      </table>
    </div>
	...
	<c:if test="${totalMoney-jd-cardmoney>0}">
	<div class="itemList20201" id="money1DIVcard"><h1>一卡通&nbsp;&nbsp;&nbsp;：</h1><h2><span class="price" id="usedJd">-¥<%=cardmoney%></span></h2></div>
	</c:if>
	<c:if test="${totalMoney-jd-cardmoney<=0}">
	<div class="itemList20201" id="money1DIVcard"><h1>一卡通&nbsp;&nbsp;&nbsp;：</h1><h2><span class="price" id="usedJd">-¥${totalMoney-jd}</span></h2></div>
	</c:if>
	<c:if test="${totalMoney-jd-cardmoney>0}">
	 <div class="itemList20201" id="money2DIV"><h1>&nbsp;应付总额：</h1><h4 id="surmeryt"><span>¥</span> ${totalMoney-jd-cardmoney}</h4></div>
	</c:if>
	<c:if test="${totalMoney-jd-cardmoney<=0}">
	  <div class="itemList20201" id="money2DIV"><h1>&nbsp;应付总额：</h1><h4 id="surmeryt"><span>¥</span> 0.00</h4></div>
	</c:if>
	<c:if test="${jd==0.0&&cardmoney==0.0}">
    ...
    <%if(cards!=null){%>
      <div class="total">共使用了 <strong id="lpk_count"><%=cards.size()%></strong>张卡可以抵扣
        ...
        <strong id="lpk_discount">${totalMoney-jd}</strong> 元
         ...
         <c:if test="${totalMoney-jd-cardmoney>=0}">
        <strong id="lpk_discount">${cardmoney }</strong> 元
         </c:if>
      </div>
    <%}else{%>  
	...
	<div class="itemList20205">
	 <c:if test="${totalMoney-jd-cardmoney>0}">
	 <div class="itemList20209" id="money2DIV2"><h1>&nbsp;应付总额：</h1><h4 id="surmeryt1"><span>¥</span> ${totalMoney-jd-cardmoney}</h4></div>
	 </c:if>
	 <c:if test="${totalMoney-jd-cardmoney<=0}">
	  <div class="itemList20209" id="money2DIV2"><h1>&nbsp;应付总额：</h1><h4 id="surmeryt1"><span>¥</span> 0.00</h4></div>
	 </c:if>
	  <c:if test="${jd==0.0&&cardmoney==0.0}">
	 <div class="itemList20209" id="money2DIV2"><h1>&nbsp;应付总额：</h1><h4 id="surmeryt1"><span>¥</span> ${totalMoney}</h4></div> 
	  </c:if>
