...
<div class="box">
  ...
  <div class="orderpay">
   ...
   <span>收货信息：</span>${customerAddress.name }&nbsp;${customerAddress.address }&nbsp;${customerAddress.street }&nbsp;${customerAddress.zipCode }&nbsp;${customerAddress.mobile }&nbsp; 
    ...
    <div class="orderpay01_text2">
   <span>送货时间：</span>
	<c:if test="${saleOrder2.deliveryDate=='0'}">工作日送</c:if>
	<c:if test="${saleOrder2.deliveryDate=='1'}">作日、双休及节假日都可以送</c:if>
	<c:if test="${saleOrder2.deliveryDate=='2'}">双休日、节假日送</c:if>
   </div>
    <div class="orderpay01_text2">
   <span>应付金额：</span><samp><span class="fonys">¥</span>${saleOrder2.orderMoney}</samp>
   </div>
   </div>
  </div>
  <div class="orderpay1">
  ...
  支付方式：
  	...
  	<h2><img  src="${ctx }/Images/bank/${saleOrder2.bankCode}.jpg" /></h2>
  	  <h3><a href="${ctx }/orderpay.do?orderId=${saleOrder2.id}&addressId=${customerAddress.id }"><!-- 重新选择 --></a></h3>
...
<div id="ordpaly" class="ordersPaylayer" style="display: none"> 
	...
	<div class="ordersPaylayer02">
		...
		<div class="ordersPaylayer02_text1">
			...
			<h3 style="display:none;">》<a href="${ctx }/orderpay.do?orderId=${saleOrder2.id}&addressId=${customerAddress.id }">返回选择其他付款方式</a></h3>
		</div>
	</div>
</div>
